...
async def notification_handler(sender, data):
    ...
    for i in range(0, len(byte_values)):
        ...
        # chunk = byte_values[i:i+4]
        # int_representation = (chunk[0] << 24) | (chunk[1] << 16) | (chunk[2] << 8) | chunk[3]  # Convert bytes to int
        int_val = int(byte_values[i])
        # float_value = struct.unpack('>f', struct.pack('>I', int_representation))[0]  # IEEE 754 conversion
        combined_value.append(int_val)
